...
fn run_engine_thread<F>(state: Arc<Mutex<SharedState>>, evaluation_function: F)
    where F: Fn(Board) -> f32 + Copy + Send + Sync + 'static, {
    thread::spawn(move || {
        ...
        if !use_negamax {
            ...
            let mut best_move: ChessMove = moves.next().expect("If there aren't any moves, something has gone quite Wrong with the driver");
            for m in moves {
            ...
            if (best_move == ChessMove::default()) {message(format!("{:?}\n{:?}\n{:?}", state.lock().unwrap(), best_move, MoveGen::new_legal(&board).len()).as_str());}
            println!(
                "{}",
                UciMessage::BestMove {
                    best_move,
